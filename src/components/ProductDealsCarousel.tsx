...
const ProductDealsCarousel: FC<ProductDealsCarouselProps> = ({ title, products }) => {
        ...
        >
          <div className="flex gap-6 pb-4 min-w-min px-2">
            {products.map((product, index) => (
              ...
              >
                <div className="relative w-72 h-48 md:w-80 md:h-56 lg:w-96 lg:h-64 rounded-lg overflow-hidden shadow-md transition-all duration-300 hover:shadow-xl hover:scale-105">
                  <Image
                    src={product.image}
                    alt={`Deal on ${title}`}
                    fill
